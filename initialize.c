...
struct ssd_info *initiation(struct ssd_info *ssd)
{
    ...
    ssd->min_lsn=0x7fffffff;  //lsn可以用来算lpn, lpn就是逻辑地址，在映射表里可以找到lpn
    ...
    //初始化通道
    ssd->channel_head=(struct channel_info*)malloc(ssd->parameter->channel_number * sizeof(struct channel_info)); //channel_head首先有地址了在这里
    ...
    //首先让channel里面的所有参数归零 (i.e.readCount, writeCount)
...
struct dram_info * initialize_dram(struct ssd_info * ssd) //dram就是缓存
{
    ...
    //SECTOR是扇区，因为操作系统写入的时候还是当HDD写入的，HDD写入的基本单位是扇区512B
...
struct ssd_info * initialize_channels(struct ssd_info * ssd )
{
    ...
    {
        //局部变量p_channel  这些局部变量是指针变量，修改它们的同时也能改变channel_head里面的内容
        ...
        //最开始的时候就读参数的时候就把chip_channel给设定好了.根据的就是parameters的参数
        p_channel->current_state = CHANNEL_IDLE;
        p_channel->next_state = CHANNEL_IDLE;
        //拿了一堆局部变量，目前这些局部变量没进行任何操作
