...
struct dram_info
{
    unsigned int dram_capacity;  //缓存容量
    int64_t current_time;

    struct dram_parameter *dram_paramters;     //缓存参数
    struct map_info *map;//映射表
    struct buffer_info *buffer;//缓存
...
{//缓存的节点大小是lpn，里面的成员group是该节点的第一个lsn
